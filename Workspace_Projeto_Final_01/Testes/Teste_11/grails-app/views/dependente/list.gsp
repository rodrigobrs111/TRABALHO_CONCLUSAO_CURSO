			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="tel" title="${message(code: 'dependente.tel.label', default: 'Tel')}" />
				...
				<g:each in="${dependenteInstanceList}" status="i" var="dependenteInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${dependenteInstance.id}">${fieldValue(bean: dependenteInstance, field: "nome")}</g:link></td>
					
						<td>${fieldValue(bean: dependenteInstance, field: "pessoa")}</td>
					
						<td>${fieldValue(bean: dependenteInstance, field: "tel")}</td>
