...
class SugestaoController {
	...
	def buscarSugestao(Integer numSugestao)
	{
		def sugestaoInstance
		
		for( int i = 0 ; i < Sugestao.list(params).size() ; i++ )
		     if( Sugestao.list(params).get( i ).numSugestao == numSugestao )
			     sugestaoInstance = Sugestao.list(params).get( i )
				 
				[sugestaoInstanceList: sugestaoInstance , sugestaoInstanceTotal: 0]   
	}
